...
public class EventSequence implements FigureLength<EventSequence>, EventComplex {
			...
			if ( ev instanceof NoteOn ) {
				...
				if ( evOnQueue != null && onTimeQueue != null ) {
					...
					ChromaticMidi n = ChromaticMidi
							.getFromCode( nc, (int) ( time - onTime ), evOn.note.getVelocity() );
					es.add( onTime, n );
	...
	public int getLength() {
		...
		for ( Iterator<Map.Entry<Long, ArrayList<Event>>> it = getMap().entrySet().iterator(); it
			...
			for ( Event e : value )
				if ( e instanceof FigureLength ) {
					d = (int) Math.max( d, key + ( (FigureLength) e ).getLength() );
				}
		}
	...
	public EventSequence setLength(int d) {
		duration = d;
