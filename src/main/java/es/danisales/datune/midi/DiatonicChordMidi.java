...
public class DiatonicChordMidi extends ChordMidi<DiatonicMidi> implements PitchDiatonic {
    ...
    */
	...
	public static void showWhatIsIt(boolean outscale, Supplier<Boolean> f, ChromaticMidi... notes) {
		ArrayList<DiatonicChordMidi> chords = ChromaticChordMidi.of( notes )
	...
	public <N extends PitchChromaticSingle, Array extends PitchChromaticChord<N>> DiatonicChordMidi(Tonality ton, Array ns) {
		...
		try {
			for (PitchChromaticSingle pcs : ns) {
				ChromaticMidi cm = ChromaticMidi.of( pcs.getChromatic(), DefaultValues.OCTAVE, DefaultValues.DURATION_CHORD, DefaultValues.VELOCITY );
				add( cm );
			}
	...
	private void chromaticFunctionProcess(ChromaticFunction t, int octave) {
		if ( t == ChromaticFunction.N6 ) {
			...
			ChromaticMidi n = tonality.get( d ).toMidi( octave, DefaultValues.DURATION_CHORD, DefaultValues.VELOCITY );
			add( n );
			...
			int i = 0;
			for (Chromatic c : cc) {
				ChromaticMidi cm = ChromaticMidi.of( c, DefaultValues.OCTAVE, DefaultValues.DURATION_CHORD, DefaultValues.VELOCITY );
				ccmArray[i++] = cm;
			}

			ChromaticChordMidi ccm = ChromaticChordMidi.of(ccmArray);
