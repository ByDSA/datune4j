package es.danisales.datune.midi;

import es.danisales.datune.diatonic.DiatonicDegree;
import es.danisales.datune.diatonic.IntervalChromatic;
import es.danisales.datune.musical.Chromatic;
import es.danisales.datune.musical.transformations.AlterationsCalculator;
import es.danisales.datune.pitch.PitchChromaticSingle;
import es.danisales.datune.tonality.Tonality;
import es.danisales.others.Codeable;

public enum PitchMidi implements PitchChromaticSingle, PitchOctaveMidi, Codeable {
	C0(0), CC0(1), D0(2), DD0(3), E0(4), F0(5), FF0(6), G0(7), GG0(8), A0(9), AA0(10), B0(11),
	C1(12), CC1(13), D1(14), DD1(15), E1(16), F1(17), FF1(18), G1(19), GG1(20), A1(21), AA1(22), B1(23),
	C2(0 + Chromatic.N * 2), CC2(1 + Chromatic.N * 2), D2(2 + Chromatic.N * 2), DD2(3 + Chromatic.N * 2), E2(4 + Chromatic.N * 2), F2(5 + Chromatic.N * 2), FF2(6 + Chromatic.N * 2), G2(7 + Chromatic.N * 2), GG2(8 + Chromatic.N * 2), A2(9 + Chromatic.N * 2), AA2(10 + Chromatic.N * 2), B2(11 + Chromatic.N * 2),
	C3(0 + Chromatic.N * 3), CC3(1 + Chromatic.N * 3), D3(2 + Chromatic.N * 3), DD3(3 + Chromatic.N * 3), E3(4 + Chromatic.N * 3), F3(5 + Chromatic.N * 3), FF3(6 + Chromatic.N * 3), G3(7 + Chromatic.N * 3), GG3(8 + Chromatic.N * 3), A3(9 + Chromatic.N * 3), AA3(10 + Chromatic.N * 3), B3(11 + Chromatic.N * 3),
	C4(0 + Chromatic.N * 4), CC4(1 + Chromatic.N * 4), D4(2 + Chromatic.N * 4), DD4(3 + Chromatic.N * 4), E4(4 + Chromatic.N * 4), F4(5 + Chromatic.N * 4), FF4(6 + Chromatic.N * 4), G4(7 + Chromatic.N * 4), GG4(8 + Chromatic.N * 4), A4(9 + Chromatic.N * 4), AA4(10 + Chromatic.N * 4), B4(11 + Chromatic.N * 4),
	C5(0 + Chromatic.N * 5), CC5(1 + Chromatic.N * 5), D5(2 + Chromatic.N * 5), DD5(3 + Chromatic.N * 5), E5(4 + Chromatic.N * 5), F5(5 + Chromatic.N * 5), FF5(6 + Chromatic.N * 5), G5(7 + Chromatic.N * 5), GG5(8 + Chromatic.N * 5), A5(9 + Chromatic.N * 5), AA5(10 + Chromatic.N * 5), B5(11 + Chromatic.N * 5),
	C6(0 + Chromatic.N * 6), CC6(1 + Chromatic.N * 6), D6(2 + Chromatic.N * 6), DD6(3 + Chromatic.N * 6), E6(4 + Chromatic.N * 6), F6(5 + Chromatic.N * 6), FF6(6 + Chromatic.N * 6), G6(7 + Chromatic.N * 6), GG6(8 + Chromatic.N * 6), A6(9 + Chromatic.N * 6), AA6(10 + Chromatic.N * 6), B6(11 + Chromatic.N * 6),
	C7(0 + Chromatic.N * 7), CC7(1 + Chromatic.N * 7), D7(2 + Chromatic.N * 7), DD7(3 + Chromatic.N * 7), E7(4 + Chromatic.N * 7), F7(5 + Chromatic.N * 7), FF7(6 + Chromatic.N * 7), G7(7 + Chromatic.N * 7), GG7(8 + Chromatic.N * 7), A7(9 + Chromatic.N * 7), AA7(10 + Chromatic.N * 7), B7(11 + Chromatic.N * 7),
	C8(0 + Chromatic.N * 8), CC8(1 + Chromatic.N * 8), D8(2 + Chromatic.N * 8), DD8(3 + Chromatic.N * 8), E8(4 + Chromatic.N * 8), F8(5 + Chromatic.N * 8), FF8(6 + Chromatic.N * 8), G8(7 + Chromatic.N * 8), GG8(8 + Chromatic.N * 8), A8(9 + Chromatic.N * 8), AA8(10 + Chromatic.N * 8), B8(11 + Chromatic.N * 8),
	C9(0 + Chromatic.N * 9), CC9(1 + Chromatic.N * 9), D9(2 + Chromatic.N * 9), DD9(3 + Chromatic.N * 9), E9(4 + Chromatic.N * 9), F9(5 + Chromatic.N * 9), FF9(6 + Chromatic.N * 9), G9(7 + Chromatic.N * 9), GG9(8 + Chromatic.N * 9), A9(9 + Chromatic.N * 9), AA9(10 + Chromatic.N * 9), B9(11 + Chromatic.N * 9),
	C10(0 + Chromatic.N * 10), CC10(1 + Chromatic.N * 10), D10(2 + Chromatic.N * 10), DD10(3 + Chromatic.N * 10), E10(4 + Chromatic.N * 10), F10(5 + Chromatic.N * 10), FF10(6 + Chromatic.N * 10), G10(7 + Chromatic.N * 10),
	MIN(C0), MAX(G10);

	private int value;

	private PitchMidi(int v) {
		value = v;
	}

	private PitchMidi(PitchMidi p) {
		value = p.value;
	}

	@Override
	public PitchMidi shiftOctave(int o) {
		return of( getCode() + Chromatic.N * o );
	}

	@Override
	public PitchMidi setOctave(int o) {
		return of( getCode() % Chromatic.N + Chromatic.N * o );
	}

	@Override
	public int getCode() {
		return value;
	}

	public boolean equals(PitchMidi p) {
		return value == p.value;
	}

	@Override
	public int getOctave() {
		return value / Chromatic.N;
	}

	public static PitchMidi of(int code) {
		PitchMidiException.check( code );
		Chromatic n = Chromatic.from( code % Chromatic.N);
		int o = code / Chromatic.N;
		return of( n, o );
	}

	public static PitchMidi of(Chromatic chromatic, int octave) {
		Chromatic chromaticWithoutAlterations = AlterationsCalculator.minAlterationsFrom(chromatic);
		switch ( octave ) {
			case 0:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C0;
					case 1:
						return PitchMidi.CC0;
					case 2:
						return PitchMidi.D0;
					case 3:
						return PitchMidi.DD0;
					case 4:
						return PitchMidi.E0;
					case 5:
						return PitchMidi.F0;
					case 6:
						return PitchMidi.FF0;
					case 7:
						return PitchMidi.G0;
					case 8:
						return PitchMidi.GG0;
					case 9:
						return PitchMidi.A0;
					case 10:
						return PitchMidi.AA0;
					case 11:
						return PitchMidi.B0;
					default:
						return null;
				}
			case 1:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C1;
					case 1:
						return PitchMidi.CC1;
					case 2:
						return PitchMidi.D1;
					case 3:
						return PitchMidi.DD1;
					case 4:
						return PitchMidi.E1;
					case 5:
						return PitchMidi.F1;
					case 6:
						return PitchMidi.FF1;
					case 7:
						return PitchMidi.G1;
					case 8:
						return PitchMidi.GG1;
					case 9:
						return PitchMidi.A1;
					case 10:
						return PitchMidi.AA1;
					case 11:
						return PitchMidi.B1;
					default:
						return null;
				}
			case 2:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C2;
					case 1:
						return PitchMidi.CC2;
					case 2:
						return PitchMidi.D2;
					case 3:
						return PitchMidi.DD2;
					case 4:
						return PitchMidi.E2;
					case 5:
						return PitchMidi.F2;
					case 6:
						return PitchMidi.FF2;
					case 7:
						return PitchMidi.G2;
					case 8:
						return PitchMidi.GG2;
					case 9:
						return PitchMidi.A2;
					case 10:
						return PitchMidi.AA2;
					case 11:
						return PitchMidi.B2;
					default:
						return null;
				}
			case 3:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C3;
					case 1:
						return PitchMidi.CC3;
					case 2:
						return PitchMidi.D3;
					case 3:
						return PitchMidi.DD3;
					case 4:
						return PitchMidi.E3;
					case 5:
						return PitchMidi.F3;
					case 6:
						return PitchMidi.FF3;
					case 7:
						return PitchMidi.G3;
					case 8:
						return PitchMidi.GG3;
					case 9:
						return PitchMidi.A3;
					case 10:
						return PitchMidi.AA3;
					case 11:
						return PitchMidi.B3;
					default:
						return null;
				}
			case 4:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C4;
					case 1:
						return PitchMidi.CC4;
					case 2:
						return PitchMidi.D4;
					case 3:
						return PitchMidi.DD4;
					case 4:
						return PitchMidi.E4;
					case 5:
						return PitchMidi.F4;
					case 6:
						return PitchMidi.FF4;
					case 7:
						return PitchMidi.G4;
					case 8:
						return PitchMidi.GG4;
					case 9:
						return PitchMidi.A4;
					case 10:
						return PitchMidi.AA4;
					case 11:
						return PitchMidi.B4;
					default:
						return null;
				}
			case 5:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C5;
					case 1:
						return PitchMidi.CC5;
					case 2:
						return PitchMidi.D5;
					case 3:
						return PitchMidi.DD5;
					case 4:
						return PitchMidi.E5;
					case 5:
						return PitchMidi.F5;
					case 6:
						return PitchMidi.FF5;
					case 7:
						return PitchMidi.G5;
					case 8:
						return PitchMidi.GG5;
					case 9:
						return PitchMidi.A5;
					case 10:
						return PitchMidi.AA5;
					case 11:
						return PitchMidi.B5;
					default:
						return null;
				}
			case 6:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C6;
					case 1:
						return PitchMidi.CC6;
					case 2:
						return PitchMidi.D6;
					case 3:
						return PitchMidi.DD6;
					case 4:
						return PitchMidi.E6;
					case 5:
						return PitchMidi.F6;
					case 6:
						return PitchMidi.FF6;
					case 7:
						return PitchMidi.G6;
					case 8:
						return PitchMidi.GG6;
					case 9:
						return PitchMidi.A6;
					case 10:
						return PitchMidi.AA6;
					case 11:
						return PitchMidi.B6;
					default:
						return null;
				}
			case 7:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C7;
					case 1:
						return PitchMidi.CC7;
					case 2:
						return PitchMidi.D7;
					case 3:
						return PitchMidi.DD7;
					case 4:
						return PitchMidi.E7;
					case 5:
						return PitchMidi.F7;
					case 6:
						return PitchMidi.FF7;
					case 7:
						return PitchMidi.G7;
					case 8:
						return PitchMidi.GG7;
					case 9:
						return PitchMidi.A7;
					case 10:
						return PitchMidi.AA7;
					case 11:
						return PitchMidi.B7;
					default:
						return null;
				}
			case 8:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C8;
					case 1:
						return PitchMidi.CC8;
					case 2:
						return PitchMidi.D8;
					case 3:
						return PitchMidi.DD8;
					case 4:
						return PitchMidi.E8;
					case 5:
						return PitchMidi.F8;
					case 6:
						return PitchMidi.FF8;
					case 7:
						return PitchMidi.G8;
					case 8:
						return PitchMidi.GG8;
					case 9:
						return PitchMidi.A8;
					case 10:
						return PitchMidi.AA8;
					case 11:
						return PitchMidi.B8;
					default:
						return null;
				}
			case 9:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C9;
					case 1:
						return PitchMidi.CC9;
					case 2:
						return PitchMidi.D9;
					case 3:
						return PitchMidi.DD9;
					case 4:
						return PitchMidi.E9;
					case 5:
						return PitchMidi.F9;
					case 6:
						return PitchMidi.FF9;
					case 7:
						return PitchMidi.G9;
					case 8:
						return PitchMidi.GG9;
					case 9:
						return PitchMidi.A9;
					case 10:
						return PitchMidi.AA9;
					case 11:
						return PitchMidi.B9;
					default:
						return null;
				}
			case 10:
				switch ( chromatic.intValue() ) {
					case 0:
						return PitchMidi.C10;
					case 1:
						return PitchMidi.CC10;
					case 2:
						return PitchMidi.D10;
					case 3:
						return PitchMidi.DD10;
					case 4:
						return PitchMidi.E10;
					case 5:
						return PitchMidi.F10;
					case 6:
						return PitchMidi.FF10;
					case 7:
						return PitchMidi.G10;
					default:
						return null;
				}
			default:
				throw new PitchMidiException( octave * Chromatic.N + chromatic.intValue() );
		}
	}

	public PitchMidi shift(int i) {
		return PitchMidi.of(getCode() + i);
	}

	public PitchMidi shift(IntervalChromatic i) {
		return shift( i.getSemitones() );
	}
	
	public static PitchMidi of(DiatonicDegree degree, Tonality tonality, int octave) {
		int code = tonality.get( degree ).intValue() + octave * Chromatic.N;
		if ( tonality.get( degree ).intValue() < tonality.getRoot().intValue() )
			code += Chromatic.N;

		return of( code );
	}

	public ChromaticMidi toMidi() {
		return ChromaticMidi.builder()
				.pitch(this)
				.build();
	}
}
